...
void ActionHelper::APT_ProcessActions(tinyxml2::XMLDocument &doc, tinyxml2::XMLElement *parent, uint8_t *actions, uint8_t *aptaptdata, AptConstData *data, uint8_t *aptbuffer)
{
	...
	do
	{
		...
		{
			ALIGN(a);
				uint32_t off1 = *(uint32_t*)a;
				char* str1 = reinterpret_cast<char*>(aptbuffer + off1);
				a += 4;
				uint32_t off2 = *(uint32_t*)a;
				char* str2 = reinterpret_cast<char*>(aptbuffer + off2);
				a += 4;
		...
		case ACTION_CONSTANTPOOL:
		{
			...
			a += 4;
				swapByteOrder2(*(uint32_t *)a);
				uint32_t cpd_off = *(uint32_t*)a;
				a += 4;
				uint32_t* cpd = reinterpret_cast<uint32_t*>(aptbuffer + cpd_off);
		...
		case ACTION_PUSHDATA:
		{
			...
			a += 4;
				uint32_t pid_off = *(uint32_t*)a;
				a += 4;
				uint32_t* pid = reinterpret_cast<uint32_t*>(aptbuffer + pid_off);
		...
		{
			ALIGN(a);
				uint32_t name_off = *(uint32_t*)a;
				char* name = reinterpret_cast<char*>(aptbuffer + name_off);
				a += 4;
			uint32_t count = *(uint32_t *)a;
			a += 4;
			uint32_t flags = *(uint32_t *)a;
			a += 4;
				uint32_t args_off = *(uint32_t*)a;
				a += 4;
				FunctionArgument* args = reinterpret_cast<FunctionArgument*>(aptbuffer + args_off);
			...
			a += 4;
			a += 4;
                        df_ptr = reinterpret_cast<uintptr_t>(a);
		}
		break;
		case ACTION_DEFINEFUNCTION:
		{
			...
			a += 4;
				uint32_t args_off = *(uint32_t*)a;
				a += 4;
				char** args = reinterpret_cast<char**>(aptbuffer + args_off);
			uint32_t size = *(uint32_t *)a;
		...
		{
			ALIGN(a);
				uint32_t str_off = *(uint32_t*)a;
				char* str = reinterpret_cast<char*>(aptbuffer + str_off);
				a += 4;
		...
		break;
                ...
                if (df_ptr + df_size == reinterpret_cast<uintptr_t>(a) && is_func)
		{
			is_func = false;
		}
