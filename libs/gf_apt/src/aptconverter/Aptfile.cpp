...
#undef fixEndian
// Use the helper from Util.hpp (handles 32-bit fields and legacy 32-bit offsets).
#define fixEndian(x) fixEndianT(x)
...
// swapByteOrder(uint32_t&) is provided by Util.hpp.
	...
	for (uint32_t i = 0; i < data->itemcount; ++i)
	{
		...
		if (data->items[i]->type == TYPE_STRING)
		{
			//read the string from the position specified
                        // strvalue is stored as a 32-bit offset in the file.
                        const uint32_t str_off = static_cast<uint32_t>(reinterpret_cast<uintptr_t>(aci->strvalue));
                        data->items[i]->strvalue = reinterpret_cast<const char*>(constbuffer + str_off);
